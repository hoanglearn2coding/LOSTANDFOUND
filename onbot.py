...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        await update.message.reply_text("🤖 Dạ, Ông chủ dùng gì ạ?")
        return
    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
    try:
        prompt = f"Bạn là trợ lý AI cao cấp của Ông chủ. Trả lời ngắn gọn: {query}"
        response = ai_model.generate_content(prompt)
        await update.message.reply_text(f"🤖 **AI:** {response.text}", parse_mode="Markdown")
    except: await update.message.reply_text("❌ Lỗi AI rồi Ông chủ.")
...
async def tnote_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        save_data(); await update.message.reply_text(f"✅ Đã lưu note cho việc {idx+1}")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pk_"):
        ...
        state["boards"][today].append({"id": fid, "home": m["teams"]["home"]["name"], "away": m["teams"]["away"]["name"], "status_icon": "⏳", "note": "", "notified": False, "score": ""})
        save_data()
        
        # Hỏi dùng Note ngay cho Trận đấu
        idx = len(state["boards"][today])
        kb = [[InlineKeyboardButton("📝 Thêm ghi chú trận đấu", callback_data=f"asknote_m_{idx}")]]
        await query.edit_message_text(f"✅ Đã pick: {m['teams']['home']['name']}", reply_markup=InlineKeyboardMarkup(kb))
...
async def history_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not matches: return await update.message.reply_text("📭 Chưa có trận kết thúc.")
    res = "📜 **HISTORY BOARD:**\n"
    for i, m in enumerate(matches):
...
async def mnote_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        save_data(); await update.message.reply_text(f"✅ Đã ghi chú cho trận số {idx+1}")
...
def main():
    ...
    if app.job_queue: app.job_queue.run_repeating(monitor, interval=120, first=10)
    print("🚀 BOT SUPREME V2 ĐÃ ONLINE!"); app.run_polling()
